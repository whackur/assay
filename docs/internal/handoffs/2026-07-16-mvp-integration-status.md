...
- `cargo test --workspace`: `assay-git`의 capability probe에 의존하는 여러
  `security_boundaries` 테스트(예:
  `rejects_malformed_or_multiline_commit_time`,
  `raw_diff_contract_accepts_chmod_type_change_and_threshold_renames`)가
  4-스레드 부하에서 같은 `ProbeCapabilities/Io` 근본 원인으로 간헐 실패한다
  (기존에 알려진 flaky, 이번 변경과 무관). `cargo test -p assay-git --
  --test-threads=1` 재실행 시 14/14 통과. 나머지 모든 크레이트는 통과.
...
- `assay-git`의 capability probe에 의존하는 여러 `security_boundaries`
  테스트가 4-스레드 부하에서 같은 `ProbeCapabilities/Io` 근본 원인으로 간헐
  실패한다(이번 변경과 무관). 단일 스레드 재실행으로 안정 통과. 근본 원인(부하
  시 capability probe I/O)은 별도 추적 대상이다.
