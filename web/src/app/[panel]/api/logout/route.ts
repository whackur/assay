...
// In SSO mode sign-out belongs to the identity provider (the cookie lives on the shared parent domain), so this endpoint is a plain 404 there. Logout deletes the server-side session record and clears the cookie. Plain HTML form post from the dashboard, so it works without client JavaScript. Lives under the secret /panel-<slug> path like every other admin surface.
