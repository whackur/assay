// Shared helpers for the route-level contract tests under src/app/[panel]/api/*.test.ts. Each test invokes the real route handlers with real NextRequest objects against a fresh ASSAY_DATA_DIR, exactly as Next calls them. Helpers stay framework-agnostic so any scenario file can compose setup, login, and catalog toggles without duplicating wiring.
...
// notFound() escapes a handler as Next's HTTP-fallback error; matching on the digest distinguishes a genuine 404 from any other rejection.
...
// With ASSAY_SSO_JWKS_URL set, the local credential surface disappears. The env flip is wrapped in try/finally so standalone-mode tests stay unaffected.
