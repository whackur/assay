...
/// Wall-clock instant in seconds since the Unix epoch, injected for determinism.
...
/// Raw upstream compact identity token. Never derives Debug, Display, or serde.
#[derive(Clone)]
...
/// Authenticated claim set a signature verifier returns before policy validation.
/// Verifier proves signature and reports the algorithm; it enforces no policy.
...
impl VerifiedClaims {
    ...
    /// Normalized values of one configured string-or-array claim.
...
impl VerifiedClaimsBuilder {
    ...
    /// Nonce bound to one authorization transaction.
    #[must_use]
