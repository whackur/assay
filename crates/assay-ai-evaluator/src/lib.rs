...
//!
//! Provider output and prose are untrusted until [`Evaluator`] validates their
//! schema-shaped fields, rubric membership, bounded ratings, and citations
//! against the exact [`EvidenceBundle`]. The validated scoring view excludes
//! provider rationale. Two provider families plug into the same boundary
//! (ADR 0012): API-key HTTP adapters over the shared [`ApiKeyEvaluator`]
//! machinery with injected [`SecretStore`] and [`HttpTransport`] ports, and
//! agentic CLI adapters over the injected [`SnapshotWorkspace`] and
//! [`AgentRunner`] ports. The crate performs no network, filesystem, process,
//! credential, or score-compilation I/O. Secret stores, snapshot materializers,
//! and process runners remain injected; every provider's untrusted bytes pass
//! through the one [`Evaluator`] validation path.
