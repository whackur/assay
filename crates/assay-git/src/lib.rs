...
//!
//! Collection implementations depend on domain contracts and must never execute
//! code from an analyzed repository.
//!
//! This crate measures facts pinned to immutable Git objects: resolved commit
//! and tree IDs, tracked entry metadata, bounded content digests, and bounded
//! history availability. It cannot verify that a project builds, works, is
//! safe, or matches an uncommitted working tree. An unavailable object is
//! missing evidence, not evidence that a file is empty or absent; a content
//! hash is provenance, not a quality or security judgment.
