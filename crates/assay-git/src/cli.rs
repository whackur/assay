...
impl GitCliAdapter {
    ...
    fn collect_parent_delta(
        ...
        let changes = match parse_raw_diff(&raw, format) {
            Ok(changes) => changes,
            Err(_) => return unavailable_parent_delta(ParentDeltaIssue::MalformedOutput),
        };
        ...
        let renamed_changes = match parse_raw_diff(&renamed, format) {
            Ok(changes) => changes,
            Err(_) => return unavailable_parent_delta(ParentDeltaIssue::MalformedOutput),
        };
        ParentDelta::new(
...
fn parse_raw_diff(
    ...
    while index < segments.len() {
        ...
        let status = parse_diff_status(&fields)?;
...
fn parse_diff_status(fields: &[&[u8]]) -> Result<u8, CollectionError> {
    let old_mode = fields[0].strip_prefix(b":").unwrap_or_default();
    let new_mode = fields[1];
    let old_null = fields[2].iter().all(|byte| *byte == b'0');
    let new_null = fields[3].iter().all(|byte| *byte == b'0');
    let old_absent = old_mode == b"000000";
    let new_absent = new_mode == b"000000";
    let status = fields[4];
    let valid = match status {
        [b'A'] => old_absent && old_null && !new_absent && !new_null,
        [b'D'] => !old_absent && !old_null && new_absent && new_null,
        [b'M'] => !old_absent && !old_null && !new_absent && !new_null,
        ...
        [b'C' | b'R', hundreds, tens, ones] => {
            !old_absent
                ...
                && !new_absent
                && !new_null
                && valid_similarity_score(*hundreds, *tens, *ones)
        }
        _ => false,
    };
    if valid {
...
fn valid_similarity_score(hundreds: u8, tens: u8, ones: u8) -> bool {
    hundreds.is_ascii_digit()
        && tens.is_ascii_digit()
        && ones.is_ascii_digit()
        && usize::from(hundreds - b'0') * 100
